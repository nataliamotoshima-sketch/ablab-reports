...
async function generateTexts({ g, m, ga4, marca, periodo, meta, mesAnt, contexto, proximos, histWhats, histTuo, histTel }) {
  ...
  const vr = (a, b) => b ? `${Math.round((a-b)/b*100):+d}%` : 'n/d'
...
async function buildPPTX({ g, m, ga4, marca, periodo, meta, mesAnt, histWhats, histTuo, histTel, texts }) {
  ...
  await editSlide(3, xml => {
    ...
    xml = r(xml, 'R$ 6.953,84', brl(g.custo)); xml = r(xml, '>42.710<', `>${num(g.impr)}<`)
    xml = r(xml, '>15,1%<', `>${g.ctr.toFixed(1)}%<`); xml = r(xml, '>R$ 1,08<', `>${brl(g.cpc)}<`)
    xml = r(xml, '>6.434<', `>${num(g.cli)}<`); xml = r(xml, '>33%<', `>${g.txc.toFixed(1)}%<`)
    xml = r(xml, '>2.275<', `>${num(g.conv)}<`); xml = r(xml, '>R$ 3,06<', `>${brl(g.cpa)}<`)
    xml = r(xml, 'R$ 1.773,62', brl(m.custo)); xml = r(xml, '>775.877<', `>${num(m.impr)}<`)
    xml = r(xml, '>1,3%<', `>${m.ctr.toFixed(2)}%<`); xml = r(xml, '>R$ 0,17<', `>${brl(m.cpc)}<`)
    xml = r(xml, '>10.322<', `>${num(m.cli)}<`); xml = r(xml, '>1%<', '>—<')
    xml = r(xml, '>13<', '>—<'); xml = r(xml, '>R$ 136,43<', '>—<')
    xml = r(xml, 'R$ 8.727,46', brl(g.custo + m.custo)); xml = r(xml, '>818.587<', `>${num(g.impr + m.impr)}<`)
    xml = r(xml, '>2,05%<', `>${((g.cli+m.cli)/(g.impr+m.impr)*100).toFixed(1)}%<`)
    xml = r(xml, '>R$ 0,52<', `>${brl((g.custo+m.custo)/(g.cli+m.cli))}<`)
    xml = r(xml, '>16.756<', `>${num(g.cli+m.cli)}<`); xml = r(xml, '>26,22%<', `>${g.txc.toFixed(1)}%<`)
    xml = r(xml, '>2.288<', `>${num(g.conv)}<`); xml = r(xml, '>R$ 3,81<', `>${brl(g.cpa)}<`)
    return xml
  })
  ...
  await editSlide(5, xml => {
    ...
    xml = r(xml, '>975<', '>1.143<'); xml = r(xml, '>736<', '>523<'); xml = r(xml, '>96<', '>146<')
    xml = r(xml, '>Mar&#xE7;o<', `>${mes_ant_cap}<`)
    xml = r(xml, '>1.143<', `>${num(histWhats)}<`); xml = r(xml, '>523<', `>${num(histTuo)}<`); xml = r(xml, '>146<', `>${num(histTel)}<`)
    xml = r(xml, '>Abril<', `>${mes_atual_cap}<`)
    xml = r(xml, '>1.078<', `>${num(ga4.whatsapp)}<`); xml = r(xml, '>1.025<', `>${num(ga4.tuotempo)}<`); xml = r(xml, '>163<', `>${num(ga4.telefone)}<`)
    return xml
  })
